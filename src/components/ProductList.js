...
const ProductList = (props) => {
    ...
    const productByPrice = useSelector((state)=>state.bySlug.slugedProduct.productByPrice)
    let {slug} = useParams()
    useEffect(()=>{
        ...
        dt.push(
            <div key={Math.random() * 10} className='card'>
                <div className='cardHeader'>
                    <p>{slug +' mobile under' }{
                    product === 'under5k' ? 
                    ' tk 5000' : product === 'under10k' ?
                     ' tk 10000' : product === 'under15k' ?
                      ' tk 15000' : product === 'under20k' ?
                       ' tk 20000' : product === 'under25k' ?
                        ' tk 25000' : "" }</p>
                            ...
                            <div className='productImgContainer'>
                             {
                                item.productPicture.map((element)=>(
                                    ...
                                    <img key={Math.random() * 10} src={`http://localhost:8000/public/${element.img}`} />
